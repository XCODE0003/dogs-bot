...
import {parseStringEtsy} from "@/handlers/parseUrl/etsy/parseStringEtsy";
...
async function preCreateAd(ctx, parse, match,deliveryPrice) {
    ...
    const domen = await getDomen(ctx.user,'etsy')
...
async function handler (ctx: Context) {
    const parse = await parseStringEtsy(ctx.msg.text, ctx)
...
export const scene = new Scene<Context>('setDeliveryAmountEtsy')

scene.always().callbackQuery(/skip setDeliveryAmountEtsy/, async ctx => {
