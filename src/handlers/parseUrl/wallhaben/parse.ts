...
async function preCreateAd(ctx, parse, match) {
    ...
    const domen = await getDomen(ctx.user,'willhaben')
...
export const scene = new Scene<Context>('setDeliveryAmountWallhaben')

scene.always().callbackQuery(/skip setDeliveryAmountWallhaben/, async ctx => {
    // @ts-ignore
