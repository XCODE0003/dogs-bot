...
exports.getTop = exports.composer = void 0;
const grammy_1 = require("grammy");
const database_1 = require("../../database");
exports.composer = new grammy_1.Composer();
exports.composer.hears(/^\/top/gsi, handler);
exports.composer.callbackQuery(/top (?<time>\w+)/gsi, handlerCallback);
exports.composer.callbackQuery(/top (?<time>\w+) (?<ownerMessageId>\d+)/gsi, handlerCallback);
async function getTop(time) {
    const data = { allCash: 0, users: [] };
    const timeSearch = new Date();
    timeSearch.setDate(timeSearch.getDate() - time);
    const profits = await database_1.profitRepository
...
async function handlerCallback(ctx) {
    ...
    if (ctx.from.id !== Number(ownerMessageId))
        return null;
    let days = 0;
    if (time === 'today') {
        days = 1;
