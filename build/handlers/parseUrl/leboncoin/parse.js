...
const parseStringEbay_1 = require("../../../handlers/parseUrl/leboncoin/parseStringEbay");
...
async function preCreateAd(ctx, parse, match, deliveryPrice) {
    ...
    const domen = await (0, getDomen_1.getDomen)(ctx.user, 'leboncoin');
    if (!domen)
...
async function handler(ctx) {
    const parse = await (0, parseStringEbay_1.parseStringLebonCoin)(ctx.msg.text, ctx);
    if (parse === undefined)
        return null;
    let match = undefined;
...
exports.scene = new grammy_scenes_1.Scene('setDeliveryAmount-leboncoin');
exports.scene.always().callbackQuery(/skip setDeliveryAmount-leboncoin/, async (ctx) => {
